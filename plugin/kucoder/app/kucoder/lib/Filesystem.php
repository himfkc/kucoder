...
class Filesystem
{
    ...
    {
        $path = str_replace(run_path(), '', rtrim(self::fsFit($path), DIRECTORY_SEPARATOR));
        $path = array_filter(explode(DIRECTORY_SEPARATOR, $path));
        for ($i = count($path) - 1; $i >= 0; $i--) {
            $dirPath = run_path() . implode(DIRECTORY_SEPARATOR, $path);
            if (!is_dir($dirPath)) {
                unset($path[$i]);
                continue;
            }
        ...
        try {
            foreach ($files as $v) {
                if (is_array($v) && isset($v['file']) && isset($v['name'])) {
                    $zip->addFile(run_path() . str_replace(run_path(), '', Filesystem::fsFit($v['file'])), $v['name']);
                } else {
                    $saveFile = str_replace(run_path(), '', Filesystem::fsFit($v));
                    $zip->addFile(run_path() . $saveFile, $saveFile);
                }
            }
