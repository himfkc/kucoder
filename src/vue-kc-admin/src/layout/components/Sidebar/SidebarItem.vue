  ...
  <div v-if="!item.hidden">
    <template
      ...
      <app-link v-if="onlyOneChild.meta" :to="resolvePath(onlyOneChild.path, onlyOneChild.query)">
        <el-menu-item :index="resolvePath(onlyOneChild.path)" :class="{ 'sub-menu-title-noDropdown': !isNest }">
          ...
          <kc-icon :class-name="onlyOneChild.meta.icon || item.meta?.icon" :icon="item.meta?.icon"
            style="height: 32px; width: 16px"></kc-icon>
    ...
    <el-sub-menu v-else ref="subMenu" :index="resolvePath(item.path)" teleported>
      <template v-if="item.meta" #title>
        ...
        <kc-icon :class-name="item.meta?.icon" :icon="item.meta.icon" style="height: 32px; width: 16px"></kc-icon>
